...
import React, { useEffect, useRef, useState, useMemo }
from "react";
import {
  motion,
  AnimatePresence,
...
import { animate, AnimationPlaybackControls } from "framer-motion"; // <--- O'ZGARTIRILDI (AnimationPlaybackControls qo'shildi)
...
export interface ThreeDImageRingProps {
  ...
  autoRotate?: boolean;
  /** Time in milliseconds between auto-rotations (e.g., 3000ms = 3 seconds) */
  autoRotateInterval?: number;
  /** Animation duration for auto-rotation (e.g., 1.5s) */
...
export function ThreeDImageRing({
  ...
  // --- YANGI PROPS DEFAULT QIYMATLARI ---
  autoRotate = false,
  ...
  const intervalRef = useRef<NodeJS.Timeout | null>(null); // <--- YANGI (Interval'ni saqlash uchun ref)
  const inertiaAnimationRef = useRef<AnimationPlaybackControls | null>(null); // <--- YANGI (Inertsiya animatsiyasini boshqarish uchun)
  ...
  // --- AVTO-AYLANISH FUNKSIYALARI ---


  const stopAutoRotate = () => { // <--- YANGI
    if (intervalRef.current) {
      clearInterval(intervalRef.current);
      intervalRef.current = null;
    }
    // Davom etayotgan inertia animatsiyasini to'xtatish
    inertiaAnimationRef.current?.stop();
  };

  const startAutoRotate = () => { // <--- YANGI
    if (!autoRotate || intervalRef.current) return; // Agar yoqilmagan bo'lsa yoki allaqachon ishlayotgan bo'lsa, chiqib ketish

    intervalRef.current = setInterval(() => {
      const currentRot = rotationY.get();
      const nextRot = currentRot - angle; // Keyingi rasmga o'tish (chapga)

      // Inertsiya animatsiyasini saqlash
      inertiaAnimationRef.current = animate(rotationY, nextRot, {
        type: "tween", // "spring" yoki "inertia" o'rniga "tween"
        duration: autoRotateDuration,
        ease: "easeOut",
        onComplete: () => {
          inertiaAnimationRef.current = null; // Animatsiya tugagach tozalash
        }
      });
    }, autoRotateInterval);
  };

  // --- useEffect O'ZGARTIRISHLARI ---

  useEffect(() => {
    // ... (rotationY.on("change") o'zgarmadi)
  ...
  // <--- YANGI useEffect (Avto-aylanishni boshqarish uchun) ---
  useEffect(() => {
    // Komponent 'mount' bo'lganda avto-aylanishni boshlash
    startAutoRotate();
  ...
  // --- DRAG FUNKSIYALARINI YANGILASH ---

  const handleDragStart = (event: React.MouseEvent | React.TouchEvent) => {
    stopAutoRotate(); // <--- YANGI (Drag boshlanganda avto-aylanishni to'xtatish)
    if (!draggable) return;

    isDragging.current = true;
    const clientX = "touches" in event ? event.touches[0].clientX : event.clientX;
    startX.current = clientX;
    rotationY.stop(); // Mavjud animatsiyani to'xtatish
    velocity.current = 0;
    if (ringRef.current) {
  ...
  const handleDragEnd = () => {
    ...
    // Inertsiya animatsiyasini saqlash
    inertiaAnimationRef.current = animate(initial, target, { // <--- O'ZGARTIRILDI
      type: "inertia",
      ...
      onComplete: () => { // <--- YANGI
        ...
        // Drag tugagandan keyin avto-aylanishni qayta boshlash
        if (!pauseOnHover) { // Agar hover'da to'xtash yoqilmagan bo'lsa
           startAutoRotate();
        }
      },
    });
  ...
  return (
    <div
      ref={containerRef}
      className={cn(
        "w-full h-screen overflow-hidden select-none relative",
        containerClassName
      )}
      style={{
        backgroundColor,
      ...
      // --- YANGI (Hover'da to'xtatish) ---
      onMouseEnter={pauseOnHover ? stopAutoRotate : undefined}
      onMouseLeave={pauseOnHover ? startAutoRotate : undefined}
      // ------------------------------------
    >
      <div
        style={{
